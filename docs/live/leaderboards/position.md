...
- The `mapUid` parameters in the URL has to correspond with a map in the request body.
...
- This endpoint may sometimes return an empty list (`[]`) even when passed valid parameters. It is not known why but it seems to be inconsistent. If you experience this issue, try to use different accounts and switch between dedicated/service accounts (see [Auth](/auth) for more information).
